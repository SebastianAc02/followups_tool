...
const CANAL_CODIGO: Record<Canal, string> = { correo: 'CO', llamada: 'LL', whatsapp: 'WA' };
...
export function TablaCuentas({ filas, conteos }: { filas: Filas; conteos: Conteos }) {
  return (
    <div className="border-r border-line">
      ...
      <div className={cx(COLS, 'px-[22px] py-[9px] font-mono text-[10px] uppercase tracking-[0.06em] text-faint')}>
        <span />
        <span>Cuenta</span>
        <span>Ciudad</span>
      ...
      {filas.length === 0 ? (
        ...
        filas.map((f) => (
          <div key={f.id} className={cx(COLS, 'border-t border-line px-[22px] py-[10px] text-[13px]')}>
            <input
              type="checkbox"
              checked
              disabled
