...
export function Sidebar({
  ...
  return (
    <div onMouseEnter={!pinned ? abrirPorHover : undefined} onMouseLeave={!pinned ? cerrarPorHover : undefined}>
      {/* franja de borde: dispara el flyout aunque el sidebar esté oculto. z-[60] a
          proposito, por encima de cualquier header sticky de pagina (ej. z-40 del
          DashboardHeader de /cola) para que nunca le robe los eventos de mouse. */}
      {!pinned && <div className="fixed left-0 top-0 z-[60] h-full w-3" />}

      <div
        className={cx(
          'flex w-[250px] flex-none flex-col border-r border-line-shell bg-shell-2 px-3 py-4 transition-transform duration-200 ease-out',
          pinned
            ? 'relative'
            : 'fixed left-0 top-0 z-50 h-full shadow-[0_0_32px_rgba(0,0,0,0.35)]',
