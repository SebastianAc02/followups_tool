-- 0026: el trigger de auditoria de `empresa`, recreado para cubrir las cinco columnas de
-- descarte que agrego la 0025. Misma mecanica y mismo motivo que la 0024: SQLite no sabe alterar
-- un trigger, se reemplaza con DROP + CREATE y ninguna fila se mueve.
--
-- Sobre `motivo_descarte` la auditoria pesa mas que sobre una columna cualquiera: sacar una cuenta
-- de la lista de llamadas es una decision comercial, y sin rastro nadie puede preguntar despues
-- quien la saco ni cuando. La prueba de cobertura (app/db/auditoria-campo.test.ts) obliga a que
-- toda columna nueva de `empresa` pase por aca.
