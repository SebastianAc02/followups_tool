"""
Migracion Campanas P4 (modo prioritaria/batch) APPLY: agrega campana.modo.
Idempotente (PRAGMA table_info antes del ALTER). No destructivo.
Log en sync_cambios con corrida=migrate-campanas-p4-<timestamp>.
"""
import os
import sqlite3
...
cur = con.cursor()
corrida = 'migrate-campanas-p4-' + datetime.now().strftime('%Y%m%d-%H%M%S')
